...
import EventOwner from './eventOwnerPage';
...
class DetailEvent extends Component{
  constructor(props){
    ...
    console.log('MAIN USER ID', this.props.profile.userObject._id);
  ...
  profile(){
    this.props.navigator.push({
      ...
      passProps: this.props.activitiesPageState.selectedActivityOwner,
      backButtonTitle: 'Event'
    });
  }
  render(){
    const {userObject} = this.props.profile;
             ...
             <View style={{flex: 1, backgroundColor: 'transparent'}}>
              ...
              <Image source={{uri: 'https://img07.deviantart.net/a85d/i/2013/022/0/3/san_francisco_city_by_tt83x-d5seu41.jpg'}}
                ...
                style={{flex:1, width:null, height:null, justifyContent:'center', opacity: 0.9, padding: 0}}>
                  ...
                  <View style={{flex: 1, justifyContent: 'flex-start', padding: 20, marginTop: 30, flexDirection: 'row'}}>
                    <View style={{flex: 1, alignItems: 'flex-start'}}>
                    <Text style={{fontSize: 20, fontWeight: '600', color: 'white', textAlign: 'left', letterSpacing:1, textShadowOffset: {width: 1, height: 1}, textShadowRadius: 0, textShadowColor: '#2A2A2A'}}>EVENT HOST</Text>
                    </View>
                    <View style={{flex: 1, alignItems: 'flex-end'}}>
                    {this.props.profile.userObject._id !== this.props.activitiesPageState.selectedActivityOwner._id ? (  <View style={{flex: 1}}><TouchableOpacity onPress={this.profile.bind(this)}>
                      <Image source={{uri: this.props.activityCreator[0].profileImg}} resizeMode="stretch"
                      style={{width:50, height:50, marginRight: 0, marginTop: 0, borderRadius: 25, borderWidth: 2, borderColor: 'white',justifyContent:'flex-end', padding: 15}}>
                      </Image>
                      </TouchableOpacity>
                      <Text style={{backgroundColor:'rgba(0,0,0,0)', textAlign:'center', color:'white', marginTop: 5, fontSize:10, fontWeight:'600', textShadowOffset: {width: 1, height: 1}, textShadowRadius: 0, textShadowColor: '#2A2A2A'}}>{this.props.activityCreator[0].firstName}</Text>
                      </View>
                    ) : null}
                    <Text></Text>
                    </View>
                  ...
                  <View style={{flex: 1, justifyContent: 'flex-end', marginBottom: 10 }}>
                    ...
                    <Text style={{fontSize: 20, fontWeight: '700', color: 'white', textAlign: 'left', opacity: 1, textAlign: 'center', marginBottom: 15, textShadowOffset: {width: 1, height: 1}, textShadowRadius: 1, textShadowColor: '#2A2A2A'}}>Details</Text>
                  </View>
